# Tekin OTG 2003 dijitalleştirme — ilerleme kütüğü (orkestratör: Claude Code)
Son güncelleme: 2026-09-11 03:01

## Durum
- Transkribe edilen sayfa: 273 / 273 — TAMAM (03:01)
- Eksik idx: yok
- Çalışan uygulayıcılar (Claude Code, 4 paralel): A 021-029+076-089+096-107 · B 109-119+126-149 · C 153-179+183-192 · D 193-209+213-239
- İkinci sistem (kullanıcının diğer ajanı) da aynı klasöre yazıyor; kural: var olan dosyayı ATLA, üzerine yazma.

## Bilinen düzeltme (bütünleştirmede yapılacak)
- idx 217 = "METİN ÖRNEKLERİ" numarasız ara kapak → idx >= 218 için BASILI SAYFA = idx + 1.
  s218-s239 başlıkları bunu belirtiyor; s240-s272 başlıklarına "(basılı NNN+1)" eklenecek. Atıflar basılı sayfaya göre.

## Sıradaki adımlar
1. [x] 273/273 tamlık; [?] sayımı; 5 rastgele sayfa PNG ile karşılaştırma
2. [x] s240-s272 başlık kayma düzeltmesi
3. [x] Tek dosya: tekin_otg_2003.md (sayfa işaretli, basılı numara ile)
4. [x] git commit (mesaj Türkçe + Co-Authored-By)
5. [ ] Kılavuz/kaynak çelişki taraması: sozluk.json (40 kelime) + §4 kararları + atıf sayfaları ↔ Tekin metni → onay listesi
6. [ ] Onaylananları §6 şablonuyla uygula; kapı + build + build:slayt
7. [ ] Ölmez 2017 (566 sf, tarama) — ikinci kitap, aynı yöntem

## Tamamlanan
- 2026-09-11 gece: pilot s.44/52/108; Ata 2011 + Tanyeri txt (commit c580d6d); ilk tur 122 sayfa (429 limitiyle kesildi); ikinci tur 4 parti başlatıldı.
- Parti A bitti (34 sf, [?] yok). Kılavuz için notlar: s025 §5 sözbaşı uzun ā yalnız āç/āç-/āt; s026 §8 sözbaşı ı/i daima yazılır; s027 §10 tāmka tek örnek; s028 §11 Yenisey kapalı ē (bēl, bēş, yēti); s077 §141 baŋa/saŋa kaynaşması; s106 §255 +nIŋ dudak uyumu dışı.
- Parti B bitti (35 sf, [?] yok). Kılavuz notları: §261-262 +(I)n uyumsuz, +nI marjinal (koragıŋnı, kunı) → kitapçıktaki "+nı = n²" iddiası gözden geçirilecek; §269 araç eki +(X)n dudak uyumuna UYAR (okun) — belirtme +(I)n ile karıştırılmamalı; §265 +DA dağılımı (r/l/n sonrası +tA); §280-282 çift biçimler (bıŋ~biŋ~miŋ, beş~biş, yeti~yiti).
- Parti D bitti (44 sf, [?] yok). Notlar: sözlükte Tekin uzun ünlüyü [āt], [āy], [āş], [āra] köşeli parantezle verir; ikili yazımlar bā-/ba-, āz-/az-; ñ (kıtañ, azkıña) ile ŋ karıştırılmamalı; §463 sayı sistemi (üç yegirmi=13, bir otuz=21); KT G1 "birki uguşum" okuması.
- Parti C bitti (37 sf; [?] yalnız s189 Toyok 16-18 baskı bozuk). Notlar: §378/440 ok/ök ünlü sonrası -k; §380-398 Yenisey acınma ünlemleri; §409 -DOk l/n/r sonrası -tok.
- TAMLIK: 273/273; [?] içeren 2 sayfa (s124, s189); s217 numarasız ara kapak; s240-272 başlıklarına basılı numara eklendi; rastgele kontrol s160 = PNG birebir.
- Tek dosya: 00_Kaynaklar/kitaplar/tekin_otg_2003.md (445 KB, 491 paragraf no).
- Adım 5-6 TAMAM: çelişki raporu uygulandı (commit ile). Açık: 4 anahtarın -ın→-in tam Tekin çizgisi; tuñukuk(k)a; 𐱁=ince r Tekin 1993 atfı.
- 2026-09-11 sabah: makine yeniden başladı. Diğer ajan tekin_otg_2003.md'yi etiketsiz sürümle ezmiş ve s177-179/s183'ü biçimsel değiştirmişti → commit 221c1e7 sürümüne geri dönüldü (idx/basılı etiketleri şart). KURAL: tekin_otg_2003.md elle düzenlenmez; sNNN.md'den yeniden üretilir.
- Adım 5 çelişki taraması yeniden başlatıldı (ilk deneme kapanışta kesildi).
